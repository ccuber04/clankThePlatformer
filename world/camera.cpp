...
void Camera::render(const Vec<float>& position, const Color& color, Vec<float> size, bool filled) const {
...
void Camera::render(const Vec<float>& position, const Sprite& sprite, Vec<float> size, bool flash) const {
    Vec<float> pixel = world_to_screen(position);
...
void Camera::render(const GameObject& obj) const {
    if (grid_toggle.on) {
        render(obj.physics.position, obj.color, obj.size, true);
    }
...
void Camera::render_game_won() {
    SDL_FRect full_screen{0.0f, 0.0f, static_cast<float>(graphics.width), static_cast<float>(graphics.height)};
    graphics.draw(full_screen, Color{0, 0, 100, 180}, true);
}
